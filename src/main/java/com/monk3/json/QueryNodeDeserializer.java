...
public class QueryNodeDeserializer extends JsonDeserializer<QueryNode> {
    ...
    public QueryNode deserialize(JsonParser parser, DeserializationContext context) throws IOException {
        ...
        Integer minimumMatch = readInteger(parser, objectNode.get("minimumMatch"));
        Boolean isNot = readBoolean(parser, objectNode.get("isNot"));
    ...
    private static Integer readInteger(JsonParser parser, JsonNode node) throws JsonMappingException {
    ...
    private static Boolean readBoolean(JsonParser parser, JsonNode node) throws JsonMappingException {
        if (node == null || node.isNull()) {
            return null;
        }
        if (!node.isBoolean()) {
    ...
    private static QueryPayload readPayload(ObjectMapper mapper, JsonNode dataNode) throws IOException {
        return mapper.treeToValue(dataNode, QueryPayload.class);
    }
}
